...
class Shapes3dDataset(data.Dataset):
    """3D Shapes dataset_single class."""

    def __init__(self, dataset_folder, fields, split=None, categories=None, no_except=True, transform=None, cfg=None):
        """Initialization of the the 3D shape dataset_single.

        Args:
            dataset_folder (str): dataset_single folder
        ...
        for c_idx, c in enumerate(categories):
            ...
            if not os.path.isdir(subpath):
                logger.warning("Category %s does not exist in dataset_single." % c)
    ...
    def __len__(self):
        """Returns the length of the dataset_single."""
        return len(self.models)

    def __getitem__(self, idx):
        """Returns an item of the dataset_single.
