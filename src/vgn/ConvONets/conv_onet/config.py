...
def get_model(cfg, device=None, dataset=None, **kwargs):
    ...
    Args:
        ...
        dataset (dataset_single): dataset_single
    """
    decoder = cfg["decoder"]
    encoder = cfg["encoder"]
    c_dim = cfg["c_dim"]
    ...
    pair = "pair" in cfg.keys() and cfg["pair"]
    ipdf = "ipdf" in cfg.keys() and cfg["ipdf"]

    if tsdf_only:
        decoders = []

    elif pair:
        decoder = "simple_local_pair"
        decoder_qual = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, out_dim=1, **decoder_kwargs)
        decoders = [decoder_qual]

    elif ipdf:
        decoder = "simple_local_ipdf"
        decoder_qual = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, out_dim=1, **decoder_kwargs)
        decoders = [decoder_qual]

    elif decoder == "simple_local_score":
        decoder_out = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, **decoder_kwargs)
        decoders = [decoder_out]


    else:
        decoder_qual = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, out_dim=1, **decoder_kwargs)
        decoder_kwargs["add_noise"] = True
        decoder_rot = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, out_dim=4, **decoder_kwargs)
        decoder_force = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, out_dim=3, **decoder_kwargs)

        decoders = [decoder_qual, decoder_rot, decoder_force]
    if cfg["decoder_tsdf"] or tsdf_only:
        decoder_tsdf = models.decoder_dict[decoder](c_dim=c_dim, padding=padding, out_dim=1, **decoder_kwargs)
    ...
    if tsdf_only:
        model = models.ConvolutionalOccupancyNetworkGeometry(decoder_tsdf, encoder, device=device)
    elif pair or ipdf:
        model = models.ConvolutionalOccupancyNetworkPair(decoders, encoder, device=device, detach_tsdf=detach_tsdf)

    elif decoder == "simple_local_score":
        model = models.ConvolutionalOccupancyNetworkScore(decoders, encoder, device=device, detach_tsdf=detach_tsdf)
    elif decoder == "simple_local_feature":
        model = models.ConvolutionalOccupancyNetworkFeature(decoders, encoder, device=device, detach_tsdf=detach_tsdf)


    else:
        model = models.ConvolutionalOccupancyNetwork(decoders, encoder, device=device, detach_tsdf=detach_tsdf)
