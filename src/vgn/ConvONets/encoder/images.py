...
class LocalImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.actvn = F.relu
        if kernel_size == 1:
            self.conv_in = nn.Conv2d(4, c_dim, 1)
        else:
            self.conv_in = nn.Conv2d(4, c_dim, kernel_size, padding=1)
    ...
    def forward(self, x):
        batch_size = x.size(0)
        device = x.device
        n_voxel = x.size(2) * x.size(3)

        # pixel 2D coordintates
        ...
        p = torch.stack([coord2, coord3], dim=4)
