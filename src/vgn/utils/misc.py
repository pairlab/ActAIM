...
def apply_dex_noise(img, gamma_shape=1000, gamma_scale=0.001, gp_sigma=0.005, gp_scale=4.0, gp_rate=0.5):
...
def apply_translational_noise(img, sigma_p=1, sigma_d=0.005):
    h, w = img.shape[:2]
    hs = np.arange(h)
    ws = np.arange(w)
