...
def affordance_visual(
    ...
    if move_center:
        z_axis = np.stack(
            [
                2 * rot_vol[:, :, :, 0] * rot_vol[:, :, :, 2] + 2 * rot_vol[:, :, :, 1] * rot_vol[:, :, :, 3],
                2 * rot_vol[:, :, :, 1] * rot_vol[:, :, :, 2] - 2 * rot_vol[:, :, :, 0] * rot_vol[:, :, :, 3],
                1 - 2 * rot_vol[:, :, :, 0] * rot_vol[:, :, :, 0] - 2 * rot_vol[:, :, :, 1] * rot_vol[:, :, :, 1],
            ],
            axis=-1,
        )
...
def plot_voxel_as_cloud(
    ...
    **kwargs
    ...
    sc = ax.scatter(
        *points.T, marker=marker, s=s, alpha=alpha, c=val, cmap=plt.cm.get_cmap("RdYlBu_r"), *args, **kwargs
    )
...
def plot_tsdf_with_grasps(
    ...
    **kwargs
    ...
    points = cloud[:, :3]
    val = cloud[:, 3]
    grasp_meshes = [grasp2mesh(grasps[idx], 1) for idx in range(len(grasps))]
    ...
    color_tsdf = cmap(val)
    color_gripper_points = np.tile(np.array([[0, 1, 0, 0.3]]), (gripper_points.shape[0], 1))
    ...
    # color = cm.coolwarm(val)
    sc = ax.scatter(*points.T, marker=marker, s=s, alpha=alpha, c=color, *args, **kwargs)
    # plt.colorbar(sc, ax=ax)
    ax.set_xlim3d(0, lim[0])
    ax.set_ylim3d(0, lim[1])
    ax.set_zlim3d(0, lim[2])
