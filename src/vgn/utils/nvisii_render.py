...
class NViSIIRenderer:
    ...
    def reset(self):
        ...
        self.set_camera(self.opt["camera"]["position"], self.opt["camera"]["look_at"])
        ...
        self.set_floor(self.opt["floor"]["texture"], self.opt["floor"]["scale"], self.opt["floor"]["position"])
    ...
    def render(self, path):
        nvisii.render_to_file(width=self.width, height=self.height, samples_per_pixel=self.spp, file_path=path)
    ...
    def set_floor(self, texture_path, scale, position):
        ...
        self.floor_texture = nvisii.texture.create_from_file(name="floor_texture", path=texture_path)
