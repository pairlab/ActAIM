...
def get_mesh_pose_dict_from_world(world, physicsClientId=0, exclude_plane=True):
    mesh_pose_dict = {}
    for uid in world.bodies.keys():
        ...
        for visual in world.p.getVisualShapeData(uid, physicsClientId):
            object_name, mesh_path, scale, pose = get_mesh_pose(visual, physicsClientId)
...
def get_mesh_pose(visual, physicsClientId):
    ...
    visual_offset = Transform(Rotation.from_quat(localVisualFrameOrientation), np.array(localVisualFramePosition))
    if linkIndex != -1:
...
def get_body_pose(body_uid, physicsClientId):
    ...
    local_transform = Transform(Rotation.from_quat(local_inertial_ori), np.array(local_inertial_pos))

    base_frame_transform = Transform(Rotation.from_quat(quaternion), np.array(position)) * local_transform.inverse()
