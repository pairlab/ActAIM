...
from affordance.models.decoder import RotPolicy, ForcePolicy, PointCritic2, ScoreCritic2, ActionPolicy, StochasticActionPolicy, GMVAEActionPolicy
...
class Wrapper(nn.Module):
    def __init__(self, args):
        ...
        self.point_critic = PointCritic2()
        ...
        self.policy = GMVAEActionPolicy()

        self.score_decode = ScoreCritic2()
    ...
    def forward(self, x, y, pos, obs, camera_id, metric_model=None):
        ...
        critic_pred = self.score_decode.forward(point_feature, rotations, force)
        rot_pred, force_pred, kl_loss_z = self.policy.forward(depth, point_feature, self.obs_encoder)

        # score_pred = self.point_critic.forward(skill, point_feature)
        point_score = self.point_critic.forward(point_feature)

        point_score_label = self.compute_point_score_label(point_feature.detach())

        return kl_loss_z, point_score, rot_pred, force_pred, critic_pred, point_score_label

    def compute_point_score_label(self, point_feature):
        point_score_label = torch.zeros(point_feature.shape[0]).to(point_feature.device)
        avg_N = 100
        for i in range(point_feature.shape[0]):
            ...
            rotations = torch.tensor(rotations, device=point_feature.device).float()
            forces = torch.from_numpy(np.random.rand(avg_N, 3)).to(device=point_feature.device).float()

            current_point_score = self.score_decode.forward(repeat_point_feature, rotations, forces)
...
def main(args):
    LOSS_KEYS = ["loss_all", "loss_kl", "loss_critic", "loss_rotation", "loss_force", "loss_score"]
...
def loss_fn(y_pred, y, augment):
    ...
    kl_loss_z, score_pred, rot_pred, force_pred, critic_pred, point_score_label = y_pred
    label, rotations, force, depth = y
    ...
    loss = kl_loss_z * 0.15 + loss_critic + (loss_rot + loss_force) * label * 5 + loss_score

    loss_dict = {
        "loss_all": loss.mean().item(),
        "loss_score": loss_score.mean().item(),
        "loss_kl": (kl_loss_z).mean().item() * 0.15,
        "loss_critic": loss_critic.mean().item(),
        "loss_force": (loss_force * label).mean().item() * 5,
        "loss_rotation": (loss_rot * label).mean().item() * 5,
    }

    # print("loss_contrast: ", (kl_loss_z).mean().item() * 0.1)
    # print("loss_critic: ", loss_critic.mean().item() * 5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--description", type=str, default="without_skill")
