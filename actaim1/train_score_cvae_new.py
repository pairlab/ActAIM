...
from affordance.models.decoder import RotPolicy, ForcePolicy, PointCritic, ScoreCritic, ActionPolicy, StochasticActionPolicy
...
class Wrapper(nn.Module):
    ...
    def forward(self, x, y, pos, obs, camera_id, metric_model=None):
        ...
        if self.encoder_name == "cic" or self.encoder_name == "curl":
            ...
            skill = torch.empty((batch_size, 64)).normal_(mean=0, std=1).cuda()
            kl_loss_z, (z_mean, z_log_var), query = self.encoder_model.forward(depth, obs, self.obs_encoder, self.eval)
        ...
        return kl_loss_z, point_score, rot_pred, force_pred, critic_pred, trans_loss, point_score_label
...
def loss_fn(y_pred, y, augment):
    ...
    kl_loss_z, score_pred, rot_pred, force_pred, critic_pred, trans_loss, point_score_label = y_pred
...
if __name__ == "__main__":
    ...
    parser.add_argument("--description", type=str, default="final")
