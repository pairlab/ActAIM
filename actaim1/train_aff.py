...
def main(args):
    ...
    # args.load_path = "./data/runs/augment=22-03-27-20-27,net=False,batch_size=giga_aff,lr=3e+01,dataset=0.0003,dataset/vgn_giga_aff_pair_41280.pt"
    if args.load_path == "":
    ...
    if args.pair or args.ipdf:
        trainer = create_trainer_pair(net, optimizer, loss_pair, metrics, device)
        evaluator = create_evaluator_pair(net, loss_pair, metrics, device)
    else:
    ...
    trainer.add_event_handler(Events.EPOCH_COMPLETED(every=1), checkpoint_handler, {args.net: net})
...
def create_train_val_loaders(batch_size, val_split, augment, pair, dataset, kwargs):
    # load the dataset
    dataset = DatasetVoxel(pair, augment=augment, cvae=True, root=dataset)
...
def prepare_batch(batch, device, pair):
    if pair:
        (pc, rotations, force), (label, depth), pos, dof, object_id, camera_id = batch
    else:
...
def create_evaluator(net, loss_fn, metrics, device):
    def _inference(_, batch):
        net.eval()
        with torch.no_grad():
            x, y, pos, dof, object_id, camera_id = prepare_batch(batch, device, True)
            y_pred = select(net(x, pos), True)
            loss, loss_dict = loss_fn(y_pred, y)
        return x, y_pred, y, loss_dict
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pair", dest="pair", default=True, action="store_true")
    parser.add_argument("--ipdf", dest="ipdf", default=False, action="store_true")
