...
def main(args):
    ...
    for epoch in range(total_epoch):
        ...
        for i, data in enumerate(train_loader):
            ...
            vq_loss, data_recon, perplexity = model(depth, inter_params)
            recon_rotations, recon_point, recon_force = data_recon

            # scale the vq loss
            vq_loss = vq_loss * 0.1 * betas[epoch]
            ...
            epoch_vq_loss += vq_loss.detach().item()
        ...
        print("epoch: ", epoch)
        print("epoch_vq_loss: ", epoch_vq_loss)
        ...
        loss_kl_values.append(epoch_vq_loss)
    ...
    """
    for i, data in enumerate(train_loader):
        x, y, pos, dof = prepare_batch(data, device)
        label, rotations, force, depth = y
        # check two directions
        # rotations = rotations[:, 0, :]
        pos = pos[:, 0, :]
    ...
    plt.scatter(proj[:, 0], proj[:, 1], alpha=0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.3)
    plt.show()
