...
def select_feat_from_hm(
    ...
    # TODO out of memory if pt_cam cast to long
    ...
    # TODO move to cpu to save space
    pt_cam_con_cpu = pt_cam_con.cpu()
    pt_cam_cpu = pt_cam.cpu()
    pt_cam_dis_cpu = 1 / (torch.sqrt(torch.sum((pt_cam_con_cpu - pt_cam_cpu) ** 2, dim=-1)) + 1e-10)
    pt_cam_wei_cpu = pt_cam_wei.unsqueeze(-1).cpu() * pt_cam_dis_cpu
    _pt_cam_wei_cpu = torch.sum(pt_cam_wei_cpu, dim=-1, keepdim=True)
    _pt_cam_wei_cpu[_pt_cam_wei_cpu == 0.0] = 1
    pt_cam_wei_cpu = pt_cam_wei_cpu / _pt_cam_wei_cpu  # [nc, npt, 4]
    # pt_cam_wei = pt_cam_wei_cpu.to(hm.device)

    hm = hm.permute(0, 2, 3, 1).view(nc, h * w, nw)  # [nc, h * w, nw]
    pt_cam_cpu = pt_cam_cpu.view(nc, 4 * npt, 2)  # [nc, 4 * npt, 2]
    pt_cam_cpu = (pt_cam_cpu[:, :, 1] * w) + pt_cam_cpu[:, :, 0]  # [nc, 4 * npt]
    pt_cam_cpu = pt_cam_cpu.to(torch.int64)
    hm_cpu = hm.cpu()
    pt_cam_val_cpu = batched_index_select(hm_cpu, dim=1, index=pt_cam_cpu)
    pt_cam_val_cpu = pt_cam_val_cpu.view(nc, npt, 4, nw)

    pt_cam_val_cpu = torch.sum(pt_cam_val_cpu * pt_cam_wei_cpu.unsqueeze(-1), dim=2)

    pt_cam_val = pt_cam_val_cpu.to(hm.device)
    pt_cam_wei = pt_cam_wei_cpu.to(hm.device)

    return pt_cam_val, None, None

    # pt_cam = pt_cam_cpu.to(hm.device)



    '''
    # return pt_cam_val, pt_cam, pt_cam_wei
    # TODO need to test memory
    pt_cam_val = batched_index_select(hm, dim=1, index=pt_cam)

    # TODO operate in GPU got memory error
