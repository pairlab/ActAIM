...
class FrankaObject(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, graphics_device, device):
        ...
        self.object_init_dof_pos = (
            to_torch(self.object_dof_lower_limits, device=self.device) * 1.0
            + to_torch(self.object_dof_upper_limits, device=self.device) * 0.0
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_franka_dofs):
            ...
            if self.physics_engine == gymapi.SIM_PHYSX:
                ...
                franka_dof_props["damping"][i] = 240
            else:
        ...
        franka_start_pose.p = gymapi.Vec3(0.0, 0.0, 3.0)
        ...
        for i in range(self.num_object_dofs):
            ...
            object_dof_props["damping"][i] = 200
    ...
    def sample_replay_buf(self, key, num_sample):
        ...
        while valid_action_num <= num_sample:
            ...
            init_pos_y = init_pos + quat_apply(candidate_rotate, y_axis * 0.1)
            init_pos_y_ = init_pos + quat_apply(candidate_rotate, y_axis * (-0.1))
            ...
            for j in range(constrain_point.shape[-1]):
                ...
                for i in range(3):
                    gripper_point_inside_bound = torch.logical_and(gripper_point_inside_bound, (
            ...
            mask = to_torch([True], device=self.device).repeat(num_sample)
            for j in range(constrain_point.shape[-1]):
                point_inside_bound = to_torch([True], device=self.device).repeat(num_sample)
                for i in range(3):
                    point_inside_bound = torch.logical_and(
                        point_inside_bound,
                        (constrain_point[:, i, j] > self.action_bounding_box[i, 0] - 0.1 * self.init_dist),
                    )
                    point_inside_bound = torch.logical_and(
                        point_inside_bound,
                        (constrain_point[:, i, j] < self.action_bounding_box[i, 1] + 0.1 * self.init_dist),
                    )
                point_outside_bound = ~point_inside_bound
                mask = torch.logical_and(mask, point_outside_bound)

            for j in range(constrain_point.shape[-1]):
                mask = torch.logical_and(mask, (constrain_point[:, 2, j] > self.action_bounding_box[2, 0] + 0.0))

            for i in range(self.action_bounding_box.shape[0]):
                mask = torch.logical_and(mask, (selected_point[:, i] > self.action_bounding_box[i, 0] - 0.02))
                mask = torch.logical_and(mask, (selected_point[:, i] < self.action_bounding_box[i, 1] + 0.02))
    ...
    def uniform_action(self, num_sample):
        ...
        while valid_action_num <= num_sample:
            ...
            # selected_point = to_torch([-0.67, 0, 0.74], device=self.device).repeat((self.num_envs * sample_scale, 1))
            ...
            mask = to_torch([True], device=self.device).repeat(self.num_envs * sample_scale)
            for j in range(constrain_point.shape[-1]):
                point_inside_bound = to_torch([True], device=self.device).repeat(self.num_envs * sample_scale)
                for i in range(3):
                    point_inside_bound = torch.logical_and(
                        point_inside_bound,
                        (constrain_point[:, i, j] > self.action_bounding_box[i, 0] - 0.1 * self.init_dist),
        ...
        # self.norm_force = to_torch([0.5, 0.5, 0.0], device=self.device).repeat(num_sample, 1)
        # self.force = to_torch([0, 0, -1], device=self.device).repeat((num_sample, 1)) * 0.5 * self.init_dist
    ...
    def viewer_camera(self, env_index, is_first, is_metric=False):
        ...
        for i in range(num_camera):
            ...
            if i == 0:
                rgb_filename = "rgb_env_cam%d.png" % (i)
    ...
    def post_physics_step(self):
        ...
        if self.viewer and self.debug_viz:
            ...
            for i in range(self.num_envs):
                ...
                init_pos_y = px + quat_apply(self.hand_rot[i], y_axis * 0.1).cpu().numpy()
                init_pos_y_ = px + quat_apply(self.hand_rot[i], y_axis * (-0.1)).cpu().numpy()
                self.gym.add_lines(
                    self.viewer,
                    self.envs[i],
                    9,
