...
class Robot(object):
    def __init__(self, env, urdf, material, open_gripper=False):
        ...
        self.end_effector_index, self.end_effector = [
            (i, l) for i, l in enumerate(self.robot.get_links()) if l.name == "panda_hand"
        ][0]
        self.hand_actor_id = self.end_effector.get_id()
        self.gripper_joints = [
            joint for joint in self.robot.get_joints() if joint.get_name().startswith("panda_finger_joint")
        ]
        self.gripper_actor_ids = [joint.get_child_link().get_id() for joint in self.gripper_joints]
        self.arm_joints = [
            joint
            for joint in self.robot.get_joints()
            if joint.get_dof() > 0 and not joint.get_name().startswith("panda_finger")
        ]
    ...
    def compute_joint_velocity_from_twist(self, twist: np.ndarray) -> np.ndarray:
        ...
        ee_jacobian[:3, :] = dense_jacobian[
            self.end_effector_index * 6 - 3 : self.end_effector_index * 6, : self.robot.dof - 2
        ]
        ee_jacobian[3:6, :] = dense_jacobian[
            (self.end_effector_index - 1) * 6 : self.end_effector_index * 6 - 3, : self.robot.dof - 2
        ]
