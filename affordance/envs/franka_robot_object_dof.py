...
class FrankaRobotObjectDof(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, graphics_device, device):
        ...
        self.franka_default_dof_pos = to_torch([0, 0, 0, -1.6, 0, 1.9, 0, 0.04, 0.04], device=self.device)
    ...
    def init_vision(self, is_first, is_metric=False, obs="tsdf", modes=False):
        ...
        for i in range(2):
            self.render()
            print("iter :", i, "dof pos: ", self.object_dof_pos)
            self.step(-1)
            print("+++++++++++iter :", i, "dof pos: ", self.object_dof_pos)
        pdb.set_trace()
        print("after render: ", self.object_dof_pos)
        if is_metric:
            for i in range(self.num_envs):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(self.num_franka_dofs):
            ...
            if self.physics_engine == gymapi.SIM_PHYSX:
                ...
                franka_dof_props["damping"][i] = 1000
            else:
        ...
        franka_start_pose.p = gymapi.Vec3(-1.8, 0.0, 0.0)
        ...
        attractor_pose.p = gymapi.Vec3(-1.4, 0, 0.5)
        # attractor_pose.r = gymapi.Quat(-0.707107, 0.0, 0.0, 0.707107)
        attractor_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0, 0, 1), math.pi / 2)

        print("attractor quaternion: ", attractor_pose.r.x, attractor_pose.r.y, attractor_pose.r.z, attractor_pose.r.w)
        ...
        for i in range(self.num_object_dofs):
            ...
            object_dof_props["damping"][i] = 400
        ...
        franka_dof_props = self.gym.get_actor_dof_properties(self.envs[0], self.frankas[0])
    ...
    def viewer_camera(self, env_index, is_first, is_metric=False):
        ...
        for i in range(num_camera):
            ...
            if i == 0:
                rgb_filename = "check_robot_dof%d.png" % (i)
    ...
    def compute_observations(self):
        print("1111111111111: ", self.object_dof_pos)
        self.gym.refresh_actor_root_state_tensor(self.sim)
        self.gym.fetch_results(self.sim, False)
        pdb.set_trace()
        self.gym.refresh_dof_state_tensor(self.sim)
        print("22222222222222: ", self.object_dof_pos)
        self.gym.refresh_dof_force_tensor(self.sim)
        self.gym.refresh_rigid_body_state_tensor(self.sim)
        self.gym.refresh_net_contact_force_tensor(self.sim)
        ...
        dof_pos_scaled = self.franka_dof_pos
    ...
    def _reset(self, env_ids):
        ...
        pos = tensor_clamp(
            ...
            + 0.0 * (torch.rand((len(env_ids), self.num_franka_dofs), device=self.device) - 0.5),
            self.franka_dof_lower_limits,
            self.franka_dof_upper_limits,
        )
        self.franka_dof_pos[env_ids, :] = pos
        ...
        self.object_dof_state[env_ids, :, 1] = torch.zeros_like(self.object_dof_state[env_ids, :, 1])
        self.object_dof_state[env_ids, :, 0] = self.object_init_dof_pos.clone()
        self.franka_dof_targets[env_ids, self.num_franka_dofs :] = self.object_dof_pos.clone()
    ...
    def update_franka(self, t):
        ...
        for i in range(self.num_envs):
            ...
            pose.p.x = 0.1 * math.sin(1.5 * t - math.pi * float(i) / self.num_envs) - 1.4
            pose.p.y = -0.1 * math.cos(1.5 * t - math.pi * float(i) / self.num_envs)
            pose.p.z = 0.5 + 0.1 * math.cos(2.5 * t - math.pi * float(i) / self.num_envs)

            pose.r = gymapi.Quat(0.707107, 0.0, 0.0, -0.707107)
            # -1.4, 0, 0.5
            # pose.r.x = 0.8 * math.sin(1.5 * t - math.pi * float(i) / self.num_envs)

            """
            print("attractor pose: ", pose.p.x, pose.p.y, pose.p.z)
            print("hand pos: ", self.hand_pos[i, 0], self.hand_pos[i, 1], self.hand_pos[i, 2])
            print("---------------------------------------------------")

            print("attractor quaternion: ", pose.r.x, pose.r.y, pose.r.z, pose.r.w)
            print("hand rot: ", self.hand_rot[i, 0], self.hand_rot[i, 1], self.hand_rot[i, 2], self.hand_rot[i, 3])
            print("---------------------------------------------------")
            """
    ...
    def pre_physics_step(self, actions):
        ...
        if t > 10:
            print("call update franka")
            self.update_franka(t)

        """
        targets = self.actions
        self.franka_dof_targets[:, :self.num_franka_dofs] = tensor_clamp(targets, self.franka_dof_lower_limits,
                                                                         self.franka_dof_upper_limits)
        franka_indices = self.franka_actor_idxs.to(torch.int32)
        self.gym.set_dof_position_target_tensor_indexed(self.sim, gymtorch.unwrap_tensor(self.franka_dof_targets),
