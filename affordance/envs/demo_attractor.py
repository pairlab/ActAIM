...
pose.r = gymapi.Quat(0.0, 0.0, 0.0)
...
def update_franka(t):
    ...
    for i in range(num_envs):
        ...
        pose.p.y = 0.2 * math.cos(1.5 * t - math.pi * float(i) / num_envs)
        pose.p.z = 0.7 + 0.1 * math.cos(2.5 * t - math.pi * float(i) / num_envs)

        # pose.r.x = 0.8 * math.sin(1.5 * t - math.pi * float(i) / num_envs)

        props = gym.get_actor_rigid_body_states(env, franka_handle, gymapi.STATE_POS)
        print("---------------------------------------------------")

        print("attractor pos: ", pose.p.x, pose.p.y, pose.p.z)
        hand_pos, hand_rot = props["pose"][:][body_dict[franka_hand]]
