...
class UNet(nn.Module):
    ...
    def __init__(
        self, num_classes, in_channels=4, depth=5, start_filts=64, up_mode="transpose", merge_mode="concat", **kwargs
    ):
        """
        Arguments:
    ...
    def forward(self, x):
        ...
        # Use soft max to compute the probability
        x_origin_shape = x.shape
        x = x.view(x.shape[0], -1)
        x = self.softmax_layer(x)
        x = x.view(x_origin_shape)
        return x
